 ...
 */
class PrivateMessageAdjustAutocompleteController extends ControllerBase implements AjaxControllerInterface {
    ...
    foreach ($accounts as $account) {
      ...
      if ($account->access('view', $this->currentUser)) {
        $user_info[] = [
          ...
          'firstname' => $account->get('field_fir')->value,
  ...
  protected function getNewInboxThreads(AjaxResponse $response) {
    ...
    if (count($inbox_threads['thread_ids'])) {
      ...
      // TODO: find treads and compare user last acees time with tread update time
