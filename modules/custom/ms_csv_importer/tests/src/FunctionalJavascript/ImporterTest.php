...
namespace Drupal\ms_csv_importer\FunctionalJavascript;
...
class ImporterTest extends JavascriptTestBase {
   ...
   * {@inheritdoc}
   */
  protected static $modules = ['ms_csv_importer', 'ms_csv_importer_test'];
  ...
  protected function setUp() {
    ...
    Node::create([
      ...
      'type' => 'ms_csv_importer_test_content',
    ])->save();
  ...
  public function testNodeCsvImporter() {
    ...
    $this->processForm('node', 'ms_csv_importer_test_content');
  ...
  public function testTaxonomyTermCsvImporter() {
    ...
    $this->processForm('taxonomy_term', 'ms_csv_importer_taxonomy');
  ...
  protected function processForm(string $entity_type, string $entity_type_bundle = NULL) {
    ...
    $page->attachFileToField('files[csv]', drupal_get_path('module', 'ms_csv_importer_test') . "/content/csv_example_{$entity_type}_test.csv");
    $assert->assertWaitOnAjaxRequest();
